...
# Sizes are 1.67x those approved on the old 1152-wide canvas (stretched 1.67x),
# so the cursor keeps its size on the unstretched screen (clinic_ui.gd, "Canvas").
		...
		for i in range(1, _trail.size()):
			...
			if pass_i == 0:
				draw_line(_trail[i - 1]["pos"], _trail[i]["pos"], Color(LASER, (1.0 - age) * 0.22),
					lerpf(33.0, 10.0, age), true)
			else:
				draw_line(_trail[i - 1]["pos"], _trail[i]["pos"], Color(1.0, 0.75, 0.70, (1.0 - age) * 0.85),
					lerpf(10.0, 3.3, age), true)
	var c := _ts.mm_to_screen(_hand)
	Art.blit(self, Art.glow(), c, Vector2(103.0, 103.0), Color(LASER, 0.45))
	Art.blit(self, Art.disc(), c, Vector2(47.0, 47.0), LASER)
	Art.blit(self, Art.disc(), c, Vector2(18.0, 18.0), Color(1.0, 0.95, 0.92))
