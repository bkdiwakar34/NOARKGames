...
#   calibration.csv — per pair: calibration result and the play lifetime
...
const CALIB_COLUMNS: Array = [
	"attempt", "pair", "a_mm", "w_mm", "caught", "timeouts", "median_mt_s", "level_p", "lifetime_s",
]
...
const REACH_COLUMNS: Array = [
	"attempt", "spoke", "angle_deg", "reach_mm", "edge_x_mm", "edge_y_mm", "light_mm", "limited_by",
]
...
func open(participant_id: String, header: Array) -> bool:
	...
	_calib = _open_csv("calibration.csv", header, CALIB_COLUMNS)
	return _targets != null and _hand != null and _reach != null and _calib != null
