...
- **Depth ambiguity for single markers.** Even with the right calibration and refinement, single-marker pose estimation has natural depth ambiguity for planar targets; `σ_z` is usually ~3–5× larger than `σ_x` or `σ_y`. A multi-marker Board would shrink that.
...
- **Other Kalman variants we considered and rejected.**
  - *EKF / UKF* — designed for non-linear systems (state involves angles, etc.). Your state is linear in position+velocity; the issue isn't non-linearity, it's the model assumption (constant velocity). Linearising harder doesn't help.
  - *Particle filter* — handles arbitrary noise distributions. Your noise is already Gaussian; you'd burn CPU on hundreds of hypotheses for no real gain.
  - *Constant-acceleration Kalman* — just shifts the wrong assumption from "velocity is constant" to "acceleration is constant", still wrong at stops.
  - *IMM (Interacting Multiple Models)* — runs multiple Kalmans in parallel (e.g. "moving" + "stationary") and weights them. The only Kalman variant that would meaningfully match One Euro's behaviour, but substantially more complex to implement and tune for an outcome that's already what One Euro gives you.
- **IMU / direct velocity input.** If a hardware speed sensor (IMU + sensor fusion) replaced One Euro's velocity *estimation* step, the filter would react instantly to motion changes — no more "filter thinks I'm still moving 100 ms after I stop" overshoot. Real engineering project (IMU mount, time-sync between camera and IMU streams, complementary or Kalman fusion code). Worth doing eventually if reach-to-hold transition lag is the dominant remaining complaint, not worth doing now for the smaller wins it would buy on this hardware.
- **Dynamic zoom / ROI tracking — not applicable.** No optical zoom hardware. Digital zoom (crop + upscale) adds no information. ROI tracking (only detect inside a window around the last-known marker location) would buy CPU headroom but not accuracy; only worth implementing if a future change makes detection significantly more expensive.
- **Marker size is well-chosen.** Current 5 cm markers at ~50 cm working distance give ~80 px per marker side. Comfortably above the ~30 px minimum for reliable detection and the ~50 px threshold for accurate pose. No reason to change the physical marker size unless you also switch to ChArUco diamonds.
- **Workspace calibration order matters.** During the 4-corner workspace calibration, the script records raw values at TL → TR → BL → BR in that order. Swapping the order at calibration time produces a transform that looks plausible but maps motion to the wrong axes. If the cursor behaves wildly after a recalibration, double-check that the corners were pressed in the correct order before debugging anything else in the tracker pipeline.
